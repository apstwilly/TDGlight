...
class DeepQNetwork:
    def __init__(self, args):
        ...
        self.memory = np.zeros(
            (self.memory_size, self.n_features * self.n_gcn_features * 2 + 2)
        )
    ...
    def _build_net(self):
        ...
        self.s = tf.placeholder(
            tf.float32, [self.n_features, self.n_gcn_features], name="s"
        )  # input State
        self.s_ = tf.placeholder(
            tf.float32, [self.n_features, self.n_gcn_features], name="s_"
        )  # input Next State
        ...
        with tf.variable_scope("q_target"):
            q_target = self.r + self.gamma * tf.math.reduce_max(self.q_next)
    ...
    def learn(self):
        ...
        for i in batch_memory:
            ...
            _, cost = self.sess.run(
                ...
                feed_dict={
                    self.s: i[: self.n_features * self.n_gcn_features].reshape(
                    ...
                    self.s_: i[self.n_features * self.n_gcn_features + 2 :].reshape(
                        self.n_features, self.n_gcn_features
                    ),
                },
            )
