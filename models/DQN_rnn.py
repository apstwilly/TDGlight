...
class DeepQNetwork:
    ...
    def get_adm(self, file, sym=False):
        ...
        sadj = sp.coo_matrix(
            (np.ones(sedges.shape[0]), (sedges[:, 1], sedges[:, 0])),
    ...
    def GCN(self, name, adm, input_s):
        ...
        gcnw1 = tf.get_variable(
            name=name + "gcnw1",
            shape=(self.n_gcn_features, gcn_layer[0]),
            initializer=tf.initializers.glorot_uniform(),
        )
        gcnw2 = tf.get_variable(
            name=name + "gcnw2",
            shape=(gcn_layer[0], gcn_layer[1]),
            initializer=tf.initializers.glorot_uniform(),
        )
        gcnw3 = tf.get_variable(
            name=name + "gcnw3",
            shape=(gcn_layer[1], gcn_layer[2]),
            initializer=tf.initializers.glorot_uniform(),
        )
    ...
    def get_net(self, input_s):
        ...
        ex = tf.reshape(input_s, [1, -1, self.n_features * self.n_gcn_features])
        gru_e = tf.keras.layers.LSTM(
            self.n_features * self.n_gcn_features,
        ...
        ex = gru_e(ex)
        ex = tf.reshape(ex, [self.n_features, self.n_gcn_features])
        ex = self.GCN("matirx_A_", self.A_hat, ex)
    ...
    def _build_net(self):
        ...
        self.s = tf.placeholder(
            tf.float32, [None, self.n_features, self.n_gcn_features], name="s"
        )  # input State
        self.s_ = tf.placeholder(
            tf.float32, [None, self.n_features, self.n_gcn_features], name="s_"
        )  # input Next State
        ...
        with tf.variable_scope("q_target"):
            q_target = self.r + self.gamma * tf.math.reduce_max(self.q_next.eval())
        ...
        with tf.variable_scope("loss"):
            self.loss = tf.reduce_mean(
                tf.squared_difference(self.q_target, self.q_eval_wrt_a, name="TD_error")
            )
    ...
    def choose_action(self, observation):
        ...
        observation = observation.reshape(-1, self.n_features, self.n_gcn_features)
    ...
    def learn(self):
        ...
        for i in batch_memory:
            _, cost = self.sess.run(
                ...
                feed_dict={
                    self.s: np.array(i[0]).reshape(
                        -1, self.n_features, self.n_gcn_features
                    ),
                    self.a: np.array([i[1][0]]),
                    self.r: np.array([i[1][1]]),
                    self.s_: np.array(i[2]).reshape(
                        -1, self.n_features, self.n_gcn_features
                    ),
                },
            )
