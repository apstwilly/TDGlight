...
def _train(RL, env, total_int, intersection, episode, total_epoch, args):
    ...
    his = []
    ob_buf = np.zeros([total_int, args.n_features*args.n_gcn_features])
    ...
    for i in range(total_int):
        observation.append(intersection[i].get_observation(env_lane_num, env_lane_wait))
    ...
    for step in tqdm(range(epoch_len)):
        ...
        if(step % args.action_period == 0):
            n_ph = []
            for i in range(total_int):
                n_ph.append(RL.choose_action(observation[i]))     
        ...
        observation_ = []
        for i in range(total_int):
            observation_.append(intersection[i].get_observation(env_lane_num, env_lane_wait))
        ...
        if(step % args.action_period == 0): 
            ...
            for i in range(total_int):
                ...
                RL.store_transition(ob_buf[i], n_ph[i], re_buf[i]-tem_r, observation_[i])

        
        observation = np.array(observation_) + np.array(observation)*args.ob_decay
...
def _test(RL, env, total_int, intersection, episode, total_epoch, args):
    env.reset(True)
    ...
    for step in tqdm(range(epoch_len)):
        ...
        for i in range(total_int):
            env.set_tl_phase(intersection[i].inter_name, intersection[i].get_light(n_ph[i])+1)
        env.next_step()       
    ...
    print('test_travel : {},  total : {},  in : {},  out : {}'.format(float(env.get_average_travel_time()),
                                                                      num_car_total,
                                                                      num_car_in,
...
def run(args, model_path):
    ...
    if args.replay:
         ...
         _test(RL, env, total_int, intersection, episode, total_epoch, args)
...
if __name__ == "__main__":
    ...
    try:    
        ...
        if args.action:
            ...
            write_reslut(model_path, 'total_action', total_action_list, graph=False, ind=False) 
